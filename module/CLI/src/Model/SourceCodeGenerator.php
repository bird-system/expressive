...
class SourceCodeGenerator extends AbstractCodeGenerator
{
    ...
    {
        $CodeGenerator = new BaseModuleBaseController($table,
            $this->config->getBaseModule(),
            $this->config->getBaseModule() . '\\' . self::NAMESPACE_CONTROLLER . '\AbstractController'
        );
    ...
    {
        $CodeGenerator = new BaseModuleBaseModel(
            $table,
            $this->config->getBaseModule(),
            $this->config->getBaseModule() . '\\' . self::NAMESPACE_MODEL . '\AbstractModel'
        );
        $CodeGenerator->setBasePath($this->getConfig()->getCodeBasePath());
        $CodeGenerator->setCodeDirectoryName(self::DIRECTORY_SOURCE);
    ...
    {
        $CodeGenerator = new BaseModuleBaseTableGateway(
            $table,
            $this->config->getBaseModule(),
            $this->config->getBaseModule() . '\\' . self::NAMESPACE_TABLEGATEWAY . '\AbstractTableGateway'
        );
        $CodeGenerator->setBasePath($this->getConfig()->getCodeBasePath());
        $CodeGenerator->setCodeDirectoryName(self::DIRECTORY_SOURCE);
